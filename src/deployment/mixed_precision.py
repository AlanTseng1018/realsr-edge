...
Run::
    ...
    python -m src.deployment.mixed_precision \
        ...
        --sensitivity results/layer_analysis/edsr_200ep/layer_analysis.csv \
...
def run_sweep(args: argparse.Namespace) -> None:
    ...
    # FP32 baseline
    set_all_modes(wrappers, "fp32")
    psnr_fp32 = evaluate_psnr(model, val_loader, device)
    print(f"\n  FP32 baseline : {psnr_fp32:.4f} dB")

    # All-INT8 baseline
    set_all_modes(wrappers, "quantize")
    psnr_int8 = evaluate_psnr(model, val_loader, device)
    print(f"  All-INT8      : {psnr_int8:.4f} dB  (drop {psnr_fp32 - psnr_int8:.4f} dB)\n")
    ...
    for n_fp32 in n_fp32_candidates:
        ...
        drop = psnr_fp32 - psnr
        recovered = (psnr - psnr_int8) / max(psnr_fp32 - psnr_int8, 1e-8) * 100
        ...
        print(f"  {tag} | PSNR {psnr:.4f} dB | drop {drop:.4f} dB | recovered {recovered:.1f}%"
              + (f"  ← {fp32_layers}" if fp32_layers else ""))

    # Save CSV
    csv_path = out_dir / "mixed_precision_sweep.csv"
    with open(csv_path, "w", newline="") as f:
        writer = csv.DictWriter(f, fieldnames=["n_fp32_layers", "psnr", "drop_vs_fp32", "recovered_pct", "fp32_layers"])
        writer.writeheader()
        for r in results:
    ...
    with open(json_path, "w") as f:
        json.dump({"fp32_baseline": round(psnr_fp32, 4),
                   "int8_baseline": round(psnr_int8, 4),
    ...
    ax.plot(ns, psnrs, marker="o", linewidth=2, color="steelblue", label="Mixed precision")
    ax.axhline(psnr_fp32, linestyle="--", color="green",  linewidth=1.2, label=f"FP32  {psnr_fp32:.2f} dB")
    ax.axhline(psnr_int8, linestyle="--", color="tomato", linewidth=1.2, label=f"All-INT8 {psnr_int8:.2f} dB")
    ...
    # Summary
    print("\n  ── Summary ──")
    print(f"  FP32 baseline  : {psnr_fp32:.4f} dB")
    print(f"  All-INT8       : {psnr_int8:.4f} dB")
    best = max(results[1:], key=lambda r: r["recovered_pct"]) if len(results) > 1 else None
    if best:
        print(f"  Best trade-off : FP32×{best['n_fp32_layers']} layers"
              f" → {best['psnr']:.4f} dB  ({best['recovered_pct']:.1f}% recovered)")
